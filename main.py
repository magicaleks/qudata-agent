        ...
        def stats_heartbeat_thread():
            ...
            while True:
                try:
                    ...
                    try:
                        container_status_enum = InstanceStatus(state.status).value
                    except ValueError:
                        container_status_enum = InstanceStatus.error.value

                    stats_data = Stats(
                        cpu_util=psutil.cpu_percent(),
                        ram_util=psutil.virtual_memory().percent,
                        instance_status=(
                            container_status_enum.value
                            if isinstance(container_status_enum, InstanceStatus)
                            else container_status_enum
                        ),
                    )

                    print(
                        f"INFO: Sending stats heartbeat. Current instance status: {stats_data.instance_status}"
                    )
        ...
        try:
            if pipe_conn.poll(5):
                ...
                if signal == "AGENT_PULSE":
                    last_pulse_time = time.time()
                elif isinstance(signal, str) and signal.startswith("AGENT_DIED"):
                    print(
...
def main_launcher():
    ...
    guardian = Process(target=run_guardian_process, args=(guardian_pipe, os.getpid()))
